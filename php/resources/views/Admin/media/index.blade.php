                                ...
                                @if(isset($result_data) && $result_data != null)
                                    @foreach($result_data as $key =>$vel)
                                        ...
                                        @elseif($vel['set_name']!="standard")
                                            <div class="sbox_1_item clearfix">
                                                <span class="l"
                                                      data="option_{{$vel['category_id']}}"><strong>{{$vel['name']}}</strong></span>
                                                <div class="m">
                                                    <ul category_id="{{$vel['category_id']}}">
                                    ...
                                    <div class="WMain2">
                                        <ul>
                                            <li style="display:block;">
                                                ...
                                                <div id="shangchuan" title="上传文档" style="display: none;">
                                                    <div class="WMain3 WMain3_2"><p><i class="LGntas"></i>稿件导入:</p>
                                                        <form id="form1" method="post">
                                                            ...
                                                            <input type="text" name="name2_2" id="name2_2"
                                                                   class="txt6"/>
                                                            <button type="button" name="upload_file" id="upload_file"
                                                                    class="txt7"/>
                                                            导入</button><br/>
                                                ...
                                                <div class="WMain3 WMain3_1"><p><i class="LGntas"></i>价格:
                                                    </p>
                                                    <div id="key_input" style="color:red">
                                                        800元
                                                    </div>
                                                </div>
...
//		$(".sbox_1_item span.l[data='" + option + "']").next(".m").find("ul li:first-child a").addClass("cur");
        ...
        $("#btn_sub").click(function () {
            ...
            $("#select_media tr[rst_id!=0]").each(function () {			//已选媒体ID 数组
                ...
                data['media_id'].push(id);
            });
            data['title'] = $("input[name=name1]").val();				//活动标题
            data['Manuscripts_attr'] = $("input[name=name2]:checked").val();		//稿件内容		1 外部连接 		2 上传文档		3 内部编辑
            data['url_line'] = $("input[name=name2_1]").val();			//稿件内容》外部连接
            data['Manuscripts'] = $("input[name=name2_2]").val();		//稿件内容》上传文档	稿件导入
            data['content'] = ue.getContent();								//稿件内容》内部编辑	内容编辑		获取编辑器的内容
            if (data['Manuscripts_attr'] == 1 && data['name2_1'] == '') {
                /* var str = $("input[name=name2_1]").prev('p').text();
                 var vt=str.replace(/[&\|\\\*^%$#@:\-]/g,"");*/
                layer.msg("外部链接不能为空");
                return
            } else if (data['Manuscripts_attr'] == 2 && data['Manuscripts'] == '') {
                layer.msg("请上传文档");
                return
            } else if (data['Manuscripts_attr'] == 3 && data['content'] == '') {
                layer.msg("请填写文章内容");
                return
            }
            data['keyword'] = $("input[name=name3]").val();				//关键字
            data['start_time'] = $("input[name=name4]").val();				//开始时间
            data['end_time'] = $("input[name=name5]").val();				//截止时间
            data['remark'] = $("textarea[name=name6]").val();			//新闻备注
            if ($("input[name=agree]").is(":checked")) {				//我已经阅读并同意云媒体交易平台习家规则
                data['confirm'] = $("input[name=agree]").val();
            } else {
                data['confirm'] = "0";
            }
