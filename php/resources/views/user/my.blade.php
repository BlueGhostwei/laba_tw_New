...
@section('footer_related')
    ...
    <script>
        ...
        define('app', function (require, exports, module) {
            ...
            /*//checkbox
             var $sw_small = $('.switch-small')
             $sw_small.each(function (k, v) {
             var sw_largeGen = new Switchery(v, {
             size: 'small',
             color: '#66bb6a',
             jackColor: '#fff',
             secondaryColor: '#eee',
             ...
             //地址联动
             $('.city-proxy').change(function () {
             var $this = $(this);
             var form = $('.city-proxy').closest('.block-content');

             if ($this[0].checked) {
             form.find('input[name="aff_prefix"]').attr('disabled', false);
             form.find('.aff_type input').attr('disabled', false);
             } else {
             form.find('input[name="aff_prefix"]').attr('disabled', 1);
             form.find('.aff_type input').attr('disabled', 1);


             }
             })

             $('.town-proxy').change(function () {
             var $this = $(this);
             var form = $('.town-proxy').closest('.block-content');
             ...
             var selNet = require('lc_sel_net');


             var addr = {
             province: "
            {{ isset($user) ? $user->province : old('province') }}",
             city: "
            {{ isset($user) ? $user->city : old('city') }}",
             county: "
            {{ isset($user) ? $user->county : old('county') }}"
             }


             function sel_net_init(cname) {
             var selNet_area = new selNet(cname, {
             net: {
             province: {
             url: '',
             data: {
             ...
             selNet_area.onChangeNet = function (data) {
             data.forEach(function (v, k) {
             var $cur = selNet_area.$cur_item
             if (v.name == addr[$cur.find('select').attr('name')]) {
             selNet_area._config.selected[selNet_area.index] = k + 1;
             }
             })
             }
             ...
             dataType: 'JSON',
             success: function (data) {
             selNet_area.update(sel_net_data(data, 'name'));
             data.forEach(function (v, k) {
             if (v.name == addr.province) {
             selNet_area._config.selected[0] = k + 1;
             }
             })
             selNet_area.initSelected();

             }
             })

             function sel_net_data(data, valName) {
             var tpl_data = [];
             data.forEach(function (v, k) {
             tpl_data.push({
             val: v[valName],
