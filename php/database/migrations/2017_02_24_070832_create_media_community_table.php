...
class CreateMediaCommunityTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('media_community', function (Blueprint $table) {
            ...
            $table->string('network',20)->nullable()->comment('网络类型');
            $table->string('Entrance_level',20)->nullable()->comment('入口级别');
            $table->string('Entrance_form',20)->nullable()->comment('入口形式');
            $table->string('coverage',20)->nullable()->comment('覆盖区域');
            $table->string('channel',20)->nullable()->comment('频道类型');
            ...
            $table->string('standard',20)->nullable()->comment('正文链接');
