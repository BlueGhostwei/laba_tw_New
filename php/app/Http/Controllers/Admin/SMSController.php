...
class SMSController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $send_type = Input::get('type');
        //判断用户是否已注册
        if (!empty($send_type) && $send_type == "register") {
        ...
        $send_SMS = $this->send_sms($mobile_number, $send_type);
        if ($send_SMS['sta'] == 0) {
 ...
 */
public
function Tohttp_requst($mob, $code)
{
    $ch = curl_init();
 ...
 */
public
function send_sms($mob, $type = null)
{
