...
//                $users[$k]['number'] = Or
    ...
    public function getUserList(){
        ...
        $username = Input::get('username');
//        dd($username);
        $users = User::orderBy('user.id','Desc')->select('user.username','user.id','user.role','user.created_at','user.created_by','user.lock','acl_user.acl_name','user.wealth','count(order.*)')->join('acl_user','user.role','acl_user.acl_id')->join('order','user.id','order.user_id')->where('user.username','like','%'.$username.'%')->paginate(10);
        foreach ($users as $k=>$v){
//            $users[$k]['created_by']=User::find($users[$k]['created_by'])->username;
            if ($users[$k]['created_by']!=0){
                $users[$k]['created_by']=User::find($users[$k]['created_by'])->username;
